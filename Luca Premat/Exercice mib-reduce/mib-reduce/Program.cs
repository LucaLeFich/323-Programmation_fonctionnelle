...
class Program
{
    static void Main()
    {
        ...
        // 0. Quantité de groseilles
        var quantiteGroseilles = marche.Aggregate(0.0, (total, item) => item.Produit == "Groseilles" ? total + item.Quantite : total);
        Console.WriteLine($"Quantité totale de groseilles: {quantiteGroseilles}");

        // 1. CA total possible pour chaque producteur
        var caTotalParProducteur = marche
            .GroupBy(item => item.Producteur)
            .Select(group => new
            {
                Producteur = group.Key,
                CA = Math.Round(group.Sum(item => item.Quantite * item.PrixParUnite))
            });
        Console.WriteLine("\nCA total possible par producteur:");
        foreach (var entry in caTotalParProducteur)
        {
            Console.WriteLine($"{entry.Producteur}: {entry.CA} chf");
        }

        // 2. Min, Max, AVG CA
        var caMin = marche.Min(item => item.Quantite * item.PrixParUnite);
        var caMax = marche.Max(item => item.Quantite * item.PrixParUnite);
        var caAvg = marche.Average(item => item.Quantite * item.PrixParUnite);
        Console.WriteLine($"\nCA Min: {caMin} chf, CA Max: {caMax} chf, CA Moyenne: {caAvg} chf");

        // 3. Le plus de noix à vendre
        var maxNoixItem = marche.Where(item => item.Produit == "Noix").OrderByDescending(item => item.Quantite).FirstOrDefault();
        Console.WriteLine($"\nLe producteur avec le plus de noix à vendre: {maxNoixItem?.Producteur} avec {maxNoixItem?.Quantite} sacs\n");

        // 4. Marchand avec le plus d'affinité avec ses produits
